...
import "./Constants";
import { EMAIL_REGISTER, EMAIL_SIGN_IN, GOOGLE_SIGN_IN, ROUTE_CONSTANTS } from "./Constants";
...
export const authenticate = (userObj, navigate, signInMethod) => {
    ...
    registerOrSignIn(userObj, auth, provider, signInMethod)
      ...
      .then((userExistsResp) => handleUserExists(userExistsResp, navigate, userAuthObj))
      .catch((error) => {
...
const doesUserExistInDb = (firebaseUserId) => {
   ...
   return getToken()
    .then((token) => fetch(`${ROUTE_CONSTANTS.API_URL}/UserExists/${firebaseUserId}`, {
    method: "GET",
    headers: {
